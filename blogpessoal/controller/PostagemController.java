...
// isso libera qualquer origem e qlqr cabeçalho entre B-e e F-e. Em produção, no
// origins coloca o end do front end.
// Origins -> libera a REQUISICAO Headers -> libera CABECALHO. Usando * libera
// de qlqr origem, em ambos
public class PostagemController {

	@Autowired
	private PostagemRepository postagemRepository;
	
	// injeção de dependência: inversão de controle, eu transfiro para o Spring a
	// responsabilidade de criar e instanciar objetos
	...
	public ResponseEntity<List<Postagem>> getAll() { // vai devolver um objeto de resposta, e no corpo vai trazer varios
	...
	public ResponseEntity<Postagem> getById(@PathVariable Long id) { // ñ pode ter 2 gets iguais. o PathVariable pega a
																		// anotação {id} e coloca no Long id
		return postagemRepository.findById(id).map(resposta -> ResponseEntity.ok(resposta)) // lambda é mto mais que um
																							// IF mas serve bem aqui,
																							// pra enxutar o codigo.
	...
	@PostMapping // Desafio 21/02 com Geraldo
	public ResponseEntity<Postagem> postPostagem(@Valid @RequestBody Postagem postagem) {
		...
		/* No IF: checando repositório Tema, verifico se existe por id dentro da postagem.getTema e só depois dentro da classe Tema, verifico o ID de Tema em get.Tema
...
//	} 
	...
	/*
	 * @PutMapping //desafio 1 public ResponseEntity<Postagem> putPostagem
	 * (@Valid @RequestBody Postagem postagem) { return
	 * postagemRepository.findById(postagem.getId()) // get puxa de um ID ja
	 * existente .map(resposta -> ResponseEntity.status(HttpStatus.OK)
